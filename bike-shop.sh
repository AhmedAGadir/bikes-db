...
# query the database like this: $($PSQL "<query_here>")
  ...
  then 
    ...
    then
      ...
      then
        ...
        BIKE_INFO_FORMATTED=$(echo $BIKE_INFO | sed 's/ |/"/') # the $() makes the bit in the bracket run in a sub shell and return the output
        # send to main menu
